...
| USB gadget/UDC | Working | U-Boot ChipIdea/ULPI fastboot responds to `fastboot getvar all`; current rebuild enables `bootm`/`abootimg`, `oem run`, console capture, nested `fastboot boot`, and block-backed `fastboot flash`/`fetch`. |
