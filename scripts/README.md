...
| Script | Purpose |
| --- | --- |
| `build-u-boot.sh` | Builds the canonical Linux DTB, PIE APPSBL U-Boot, a non-flashing Android `fastboot boot` image, a block-aligned MBN for `fastboot flash UEFI`, and a partition-sized FlashApp raw `UEFI` image. |
