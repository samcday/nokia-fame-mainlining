...
| Reset hold | Android4Lumia LK's MSM8960 platform header defines `TLMM_BASE_ADDR` as `0x00800000` and `MSM_PSHOLD_CTL_SU` as `TLMM_BASE_ADDR + 0x820`, i.e. `0x00800820`; its MSM8960 target reset paths write zero to that register for reboot. A staged U-Boot PIE build using the matching `qcom,pshold` node rebooted successfully back to persistent U-Boot fastboot via `reset`. | `out/fame/android4lumia-lk-msm8227-src/platform/msm8960/include/platform/iomap.h:73-79`, `out/fame/android4lumia-lk-msm8227-src/target/msm8960/init.c:90,225-228`, live staged U-Boot reset test 2026-05-21 | C/E, live-tested through U-Boot |
