...
| Reset hold | Android4Lumia LK's MSM8960 platform header defines `TLMM_BASE_ADDR` as `0x00800000` and `MSM_PSHOLD_CTL_SU` as `TLMM_BASE_ADDR + 0x820`, i.e. `0x00800820`; its MSM8960 target reset paths write zero to that register for reboot. A staged U-Boot PIE build using a `qcom,pshold` node rebooted successfully back to persistent U-Boot fastboot via `reset`, but Linux TLMM now owns the overlapping `0x00800000..0x00803fff` resource. Mainline `pinctrl-msm` registers a restart/poweroff handler for SoCs whose pinctrl data exposes `ps_hold`, and `pinctrl-msm8960` maps `ps_hold` to GPIO108. Linux keeps PS_HOLD owned by the `qcom,msm8960-pinctrl` TLMM node rather than a separate overlapping `qcom,pshold` node, but does **not** put a self-referential default `ps_hold` state on the TLMM provider: a 2026-05-25 test left downstream TLMM consumers (`panel-reset-state`, `sdcc1-sleep-state`) stuck in deferred probe. U-Boot consumes the same `qcom,msm8960-pinctrl` node and applies the `0x820` TLMM offset for sysreset. | `out/fame/android4lumia-lk-msm8227-src/platform/msm8960/include/platform/iomap.h:73-79`, `out/fame/android4lumia-lk-msm8227-src/target/msm8960/init.c:90,225-228`, live staged U-Boot reset test 2026-05-21, `linux/drivers/pinctrl/qcom/pinctrl-msm.c:1485-1522`, `linux/drivers/pinctrl/qcom/pinctrl-msm8960.c:790-792,1016-1017,1167-1168`, `linux/arch/arm/boot/dts/qcom/qcom-msm8227.dtsi:125-133`, `u-boot/drivers/sysreset/sysreset_qcom-pshold.c:17,50-53` | B/C/E, live-tested through U-Boot |
...
| CPU SMP ACC/SAW shape | The Samsung Express MSM8930 sibling models the dual Krait CPUs with `enable-method = "qcom,kpss-acc-v1"`, per-CPU `qcom,acc` and `qcom,saw` phandles, the CPU interface PPI 14, KPSS ACC v1 blocks at `0x02088000`/`0x02098000` with alias `0x02008000`, and MSM8960 SAW2 CPU blocks at `0x02089000`/`0x02099000` with alias `0x02009000`. Mainline already provides the matching `qcom,kpss-acc-v1` boot method and bindings for these compatibles. | `samsung-expressltexx:arch/arm/boot/dts/qcom/qcom-msm8930.dtsi:70-205`, `linux/arch/arm/mach-qcom/platsmp.c:135-211,390-397`, `linux/Documentation/devicetree/bindings/clock/qcom,kpss-acc-v1.yaml:20-55`, `linux/Documentation/devicetree/bindings/soc/qcom/qcom,saw2.yaml:24-64` | E |
...
| Button resources | ACPI `BTN0` HID `QCOM0D60`, CID `PNP0C40`, exposes five button resources: `PWIO` logical pin 16 for power, plus PMIC interrupt IDs 194/199/202/201 mapping to PM8038 GPIO3/GPIO8/GPIO11/GPIO10. ACPI flags are ActiveBoth; PMIC GPIO3/8/10 are wake-capable, GPIO11 focus is not; PMIC GPIO buttons are asserted-low by default, while `PWIO._DSM` marks logical pin 16 asserted-high. | `notes/regulators-gpio.md` | A for ACPI pin/flag decode, C for old DTS function labels |
...
| Keys | Not currently modeled in the minimal first-boot DTS; stock ACPI button resources are decoded in `notes/regulators-gpio.md` | A/C, pending DTS modeling and live input validation |
