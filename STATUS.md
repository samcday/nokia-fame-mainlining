...
| U-Boot | Working | Raw APPSBL U-Boot reaches a UART prompt at `0x88F00000`; LK-chain U-Boot fastboot works at `0x80208000`; current rebuild has working `oem run`/`oem console`, `bootm`/`abootimg`, U-Boot-to-U-Boot `fastboot boot` chaining, and read-only eMMC/GPT probing. | Use the chainable U-Boot fastboot path for non-persistent hardware probing; keep write-capable MMC commands disabled unless explicitly needed. |
