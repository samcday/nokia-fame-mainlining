...
| U-Boot | Working | Raw APPSBL U-Boot reaches a UART prompt at `0x88F00000`; LK-chain U-Boot fastboot works at `0x80208000` and now has `oem run` plus `oem console` enabled. | Use `fastboot oem run:<cmd>` and `fastboot oem console` for non-persistent hardware probing from LK-chain U-Boot. |
