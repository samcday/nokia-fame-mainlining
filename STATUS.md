...
| U-Boot | Working | Raw APPSBL U-Boot reaches a UART prompt at `0x88F00000`; LK-chain U-Boot fastboot works at `0x80208000`; current rebuild has working `oem run`/`oem console`, `bootm`/`abootimg`, U-Boot-to-U-Boot `fastboot boot` chaining, eMMC/GPT probing, block-backed `fastboot flash`, and read-only `fastboot fetch`. | Keep destructive fastboot flash/erase/GPT operations disabled by procedure unless explicitly requested for a specific target. |
